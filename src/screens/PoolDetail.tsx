...
import { getPool, getPoolParticipants, joinPool, poolCheckin, getPoolCheckins } from '../lib/poolStorage'
    ...
    async function loadPoolData() {
      ...
      try {
        ...
        if (poolData) {
          ...
          const hasJoined = participantsData.some(p => p.wallet_address === walletAddress)
          setJoined(hasJoined)

          // Check if already checked in today
          const today = new Date().toISOString().split('T')[0]
          const checkedToday = checkinsData.some((c: any) => c.check_in_date === today)
  ...
  return (
    <div style={{ padding: '0 20px 40px', maxWidth: '600px', width: '100%', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
        ...
        <StampBadge state={pool.status === 'active' ? 'staked' : 'fulfilled'} size="md" />
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--rule)', borderRadius: '10px', padding: '20px', marginBottom: '8px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
          <div>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '11px', color: 'var(--text-muted)', margin: '0 0 4px', letterSpacing: '0.04em' }}>TOTAL POT</p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '18px', color: 'var(--gold)', margin: 0 }}>{totalPot.toFixed(2)} {pool.currency}</p>
          </div>
          <div>
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '11px', color: 'var(--text-muted)', margin: '0 0 4px', letterSpacing: '0.04em' }}>YOUR SHARE IF WIN</p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '18px', color: 'var(--green)', margin: 0 }}>{winnerShare.toFixed(2)} {pool.currency}</p>
          </div>
        </div>
      </div>
          ...
          {participants.map((participant, i) => (
            <div
              ...
              style={{
                ...
                alignItems: 'center',
                padding: '10px 0',
                borderBottom: i < participants.length - 1 ? '1px solid var(--rule)' : 'none',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <p style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', color: 'var(--text)', margin: 0 }}>
                    {participant.wallet_address}
                  </p>
                  <p style={{ fontFamily: 'var(--font-sans)', fontSize: '11px', color: 'var(--text-muted)', margin: '2px 0 0' }}>
              ...
              <div style={{
                ...
                border: `1px solid ${participant.status === 'active' ? 'var(--green)' : 'var(--red)'}`,
                color: participant.status === 'active' ? 'var(--green)' : 'var(--red)',
                fontFamily: 'var(--font-sans)',
                fontSize: '10px',
                fontWeight: 500,
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--rule)', borderRadius: '10px', padding: '20px', marginBottom: '14px' }}>
        ...
        <ul style={{ fontFamily: 'var(--font-sans)', fontSize: '13px', color: 'var(--text)', margin: 0, paddingLeft: '20px', lineHeight: 1.7 }}>
          <li style={{ marginBottom: '8px' }}>Each participant stakes {parseFloat(pool.stake_amount).toFixed(2)} {pool.currency}</li>
          <li style={{ marginBottom: '8px' }}>Complete all {pool.duration} days to be a winner</li>
          <li style={{ marginBottom: '8px' }}>Winners split {pool.winner_split}% of forfeited stakes</li>
