...
type Step = 'habit' | 'stake' | 'charity' | 'confirm'

const STEPS: Step[] = ['habit', 'stake', 'charity', 'confirm']
const STEP_LABELS: Record<Step, string> = {
  habit: 'Define Habit',
  stake: 'Set Stake',
  charity: 'Pick Charity',
  confirm: 'Confirm',
}

export default function NewHabit({ onCancel, onCreate }: NewHabitProps) {
  ...
  return (
    <div style={{ padding: '0 20px 40px', maxWidth: '600px', width: '100%', margin: '0 auto' }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--rule)', borderRadius: '10px', padding: '24px', marginBottom: '14px' }}>
        ...
        {step === 'habit' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div>
              ...
              <textarea
                placeholder="Describe the specific habit you will do daily"
                value={description}
        ...
        {step === 'confirm' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {[
              { label: 'Habit', value: title || 'Morning Pages' },
              { label: 'Duration', value: `${duration} days` },
              { label: 'Stake', value: `${amount.toLocaleString()} ${currency}` },
              { label: 'Charity on failure', value: charity ? charityNames[charity] : '—' },
            ].map(row => (
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--rule)', paddingBottom: '12px' }}>
                ...
                <span style={{ fontFamily: row.label === 'Stake' ? 'var(--font-mono)' : 'var(--font-sans)', fontSize: '14px', color: row.label === 'Stake' ? 'var(--gold)' : 'var(--text)', fontWeight: 500 }}>
                  {row.value}
                </span>
              </div>
            ))}
            ...
            <p style={{ fontFamily: 'var(--font-sans)', fontSize: '12px', color: 'var(--text-muted)', lineHeight: 1.7, margin: 0 }}>
              By confirming, you authorize the Soroban contract to lock <strong style={{ color: 'var(--text)' }}>{amount.toLocaleString()} {currency}</strong> from your Freighter wallet. You will get it back if you complete {duration} consecutive days.
            </p>
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        >
          {step === 'confirm' ? 'Lock Stake on Soroban →' : 'Continue →'}
        </button>
      </div>
    </div>
  )
